import { n as require_react, t as __commonJS } from "./chunk-HYFFLJFY-ByrqQPlY.js";
//#region chunk-MJ6WPKUF.js
	...
	(function() {
		...
		function unstable_scheduleCallback(priorityLevel, callback, options) {
			...
			if (typeof options === "object" && options !== null) {
				...
				if (typeof delay === "number" && delay > 0) startTime2 = currentTime + delay;
	...
	(function() {
		...
		function updateOptions(node, multiple, propValue, setDefaultSelected) {
			...
			if (multiple) {
				...
				for (var _i = 0; _i < options2.length; _i++) {
					...
					if (options2[_i].selected !== selected) options2[_i].selected = selected;
					if (selected && setDefaultSelected) options2[_i].defaultSelected = true;
				}
			} else {
				...
				for (var _i2 = 0; _i2 < options2.length; _i2++) {
					if (options2[_i2].value === _selectedValue) {
						...
						if (setDefaultSelected) options2[_i2].defaultSelected = true;
						return;
					}
					if (defaultSelected === null && !options2[_i2].disabled) defaultSelected = options2[_i2];
				}
		...
		function setValueForStyles(node, styles) {
			...
			for (var styleName in styles) {
				...
				if (isCustomProperty) style2.setProperty(styleName, styleValue);
				else style2[styleName] = styleValue;
			}
		}
		...
		validateProperty$1 = function(tagName, name, value, eventRegistry) {
			...
			if (eventRegistry != null) {
				...
				if (registrationNameDependencies2.hasOwnProperty(name)) return true;
		...
		if (typeof window !== "undefined" && typeof window.dispatchEvent === "function" && typeof document !== "undefined" && typeof document.createEvent === "function") {
			...
			invokeGuardedCallbackImpl = function invokeGuardedCallbackDev(name, func, context, a, b, c, d, e, f) {
				...
				function handleWindowError(event) {
					error2 = event.error;
					didSetError = true;
					if (error2 === null && event.colno === 0 && event.lineno === 0) isCrossOriginError = true;
					if (event.defaultPrevented) {
						if (error2 != null && typeof error2 === "object") try {
				...
				if (didCall && didError) {
					if (!didSetError) error2 = /* @__PURE__ */ new Error(`An error was thrown inside one of your components, but React doesn't know what it was. This is likely due to browser flakiness. React does its best to preserve the "Pause on exceptions" behavior of the DevTools, which requires some DEV-mode only tricks. It's possible that these don't work in your browser. Try triggering the error in production mode, or switching to a modern browser. If you suspect that this is actually an issue with React, please file an issue.`);
					else if (isCrossOriginError) error2 = /* @__PURE__ */ new Error("A cross-origin error was thrown. React doesn't have access to the actual error object in development. See https://reactjs.org/link/crossorigin-error for more information.");
					this.onError(error2);
				}
		...
		function getSuspenseInstanceFromFiber(fiber) {
			if (fiber.tag === SuspenseComponent) {
				...
				if (suspenseState === null) {
					...
					if (current2 !== null) suspenseState = current2.memoizedState;
				}
		...
		function onCommitRoot(root2, eventPriority) {
			if (injectedHook && typeof injectedHook.onCommitFiberRoot === "function") try {
				...
				} else injectedHook.onCommitFiberRoot(rendererID, root2, void 0, didError);
			} catch (err) {
				if (!hasLoggedError) {
					hasLoggedError = true;
					error("React instrumentation encountered an error: %s", err);
		...
		function markStarvedLanesAsExpired(root2, currentTime) {
			...
			while (lanes > 0) {
				...
				if (expirationTime === NoTimestamp) {
					if ((lane & suspendedLanes) === NoLanes || (lane & pingedLanes) !== NoLanes) expirationTimes[index2] = computeExpirationTime(lane, currentTime);
				} else if (expirationTime <= currentTime) root2.expiredLanes |= lane;
				lanes &= ~lane;
			}
		}
		...
		function markRootEntangled(root2, entangledLanes) {
			...
			while (lanes) {
				var index2 = pickArbitraryLaneIndex(lanes);
				var lane = 1 << index2;
				if (lane & entangledLanes | entanglements[index2] & entangledLanes) entanglements[index2] |= entangledLanes;
				lanes &= ~lane;
			}
		}
		function getBumpedLaneForHydration(root2, renderLanes2) {
			...
			if ((lane & (root2.suspendedLanes | renderLanes2)) !== NoLane) return NoLane;
			return lane;
		}
		function addFiberToLanesMap(root2, fiber, lanes) {
			...
			while (lanes > 0) {
				...
				var lane = 1 << index2;
				pendingUpdatersLaneMap[index2].add(fiber);
				lanes &= ~lane;
			}
		}
		...
		function isRootDehydrated(root2) {
			return root2.current.memoizedState.isDehydrated;
		}
			...
			if (targetInst !== null) {
				...
				if (nearestMounted === null) targetInst = null;
					...
					if (tag === SuspenseComponent) {
						...
						var root2 = nearestMounted.stateNode;
						if (isRootDehydrated(root2)) return getContainerFromFiber(nearestMounted);
						targetInst = null;
		...
		function hideInstance(instance) {
			...
			if (typeof style2.setProperty === "function") style2.setProperty("display", "none", "important");
		...
		function getMaskedContext(workInProgress2, unmaskedContext) {
			var contextTypes = workInProgress2.type.contextTypes;
			if (!contextTypes) return emptyContextObject;
			var instance = workInProgress2.stateNode;
			if (instance && instance.__reactInternalMemoizedUnmaskedChildContext === unmaskedContext) return instance.__reactInternalMemoizedMaskedChildContext;
			var context = {};
			for (var key in contextTypes) context[key] = unmaskedContext[key];
			checkPropTypes(contextTypes, context, "context", getComponentNameFromFiber(workInProgress2) || "Unknown");
			if (instance) cacheContext(workInProgress2, unmaskedContext, context);
			return context;
		}
		...
		function pushMaterializedTreeId(workInProgress2) {
			warnIfNotHydrating();
			if (workInProgress2.return !== null) {
				var numberOfForks = 1;
				var slotIndex = 0;
		...
		function queueHydrationError(error2) {
			if (hydrationErrors === null) hydrationErrors = [error2];
			else hydrationErrors.push(error2);
		}
		...
		function coerceRef(returnFiber, current2, element) {
			...
			if (mixedRef !== null && typeof mixedRef !== "function" && typeof mixedRef !== "object") {
				...
				if (element._owner) {
					...
					if (current2 !== null && current2.ref !== null && typeof current2.ref === "function" && current2.ref._stringRef === stringRef) return current2.ref;
		...
		function ChildReconciler(shouldTrackSideEffects) {
			...
			function updateElement(returnFiber, current2, element, lanes) {
				...
				if (elementType === REACT_FRAGMENT_TYPE) return updateFragment2(returnFiber, current2, element.props.children, lanes, element.key);
				if (current2 !== null) {
			...
			function updateFromMap(existingChildren, returnFiber, newIdx, newChild, lanes) {
				...
				if (typeof newChild === "object" && newChild !== null) {
					...
					if (isArray(newChild) || getIteratorFn(newChild)) return updateFragment2(returnFiber, existingChildren.get(newIdx) || null, newChild, lanes, null);
					throwOnInvalidObjectType(returnFiber, newChild);
				}
				if (typeof newChild === "function") warnOnFunctionType(returnFiber);
				return null;
		...
		function cloneChildFibers(current2, workInProgress2) {
			if (current2 !== null && workInProgress2.child !== current2.child) throw new Error("Resuming work not yet implemented.");
			if (workInProgress2.child === null) return;
		...
		function scheduleContextWorkOnParentPath(parent, renderLanes2, propagationRoot) {
			...
			while (node !== null) {
				...
				if (!isSubsetOfLanes(node.childLanes, renderLanes2)) {
					...
					if (alternate !== null) alternate.childLanes = mergeLanes(alternate.childLanes, renderLanes2);
				} else if (alternate !== null && !isSubsetOfLanes(alternate.childLanes, renderLanes2)) alternate.childLanes = mergeLanes(alternate.childLanes, renderLanes2);
		...
		function propagateContextChange_eager(workInProgress2, context, renderLanes2) {
			...
			if (fiber !== null) fiber.return = workInProgress2;
			while (fiber !== null) {
				...
				if (list !== null) {
					...
					while (dependency !== null) {
						if (dependency.context === context) {
							...
							var alternate = fiber.alternate;
							if (alternate !== null) alternate.lanes = mergeLanes(alternate.lanes, renderLanes2);
				...
				} else if (fiber.tag === ContextProvider) nextFiber = fiber.type === workInProgress2.type ? null : fiber.child;
					...
					if (_alternate !== null) _alternate.lanes = mergeLanes(_alternate.lanes, renderLanes2);
		...
		function prepareToReadContext(workInProgress2, renderLanes2) {
			...
			if (dependencies !== null) {
				if (dependencies.firstContext !== null) {
					if (includesSomeLane(dependencies.lanes, renderLanes2)) markWorkInProgressReceivedUpdate();
		...
		function cloneUpdateQueue(current2, workInProgress2) {
			...
			if (queue === currentQueue) workInProgress2.updateQueue = {
		...
		function getStateFromUpdate(workInProgress2, queue, update, prevState, nextProps, instance) {
			switch (update.tag) {
				case ReplaceState:
					...
					return payload;
				case CaptureUpdate: workInProgress2.flags = workInProgress2.flags & ~ShouldCapture | DidCapture;
				case UpdateState:
		...
		function registerMutableSourceForHydration(root2, mutableSource) {
			...
			if (root2.mutableSourceEagerHydrationData == null) root2.mutableSourceEagerHydrationData = [mutableSource, version];
			else root2.mutableSourceEagerHydrationData.push(mutableSource, version);
		}
		...
		function renderWithHooks(current2, workInProgress2, Component, props, secondArg, nextRenderLanes) {
			...
			if (current2 !== null && current2.memoizedState !== null) ReactCurrentDispatcher$1.current = HooksDispatcherOnUpdateInDEV;
			...
			hookTypesDev = null;
			hookTypesUpdateIndexDev = -1;
			if (current2 !== null && (current2.flags & StaticMask) !== (workInProgress2.flags & StaticMask) && (current2.mode & ConcurrentMode) !== NoMode) error("Internal React error: Expected static flag was missing. Please notify the React team.");
		...
		function bailoutHooks(current2, workInProgress2, lanes) {
			...
			if ((workInProgress2.mode & StrictEffectsMode) !== NoMode) workInProgress2.flags &= ~(MountPassiveDev | MountLayoutDev | Passive | Update);
			else workInProgress2.flags &= ~(Passive | Update);
		...
		function updateWorkInProgressHook() {
			...
			if (currentHook === null) {
				...
				if (current2 !== null) nextCurrentHook = current2.memoizedState;
		...
		function updateReducer(reducer, initialArg, init) {
			...
			if (pendingQueue !== null) {
				...
				if (current2.baseQueue !== baseQueue) error("Internal error: Expected work-in-progress queue to be a clone. This is a bug in React.");
		...
		function mountSyncExternalStore(subscribe, getSnapshot, getServerSnapshot) {
			...
			if (getIsHydrating()) {
				...
				if (root2 === null) throw new Error("Expected a work-in-progress root. This is a bug in React. Please file an issue.");
				if (!includesBlockingLane(root2, renderLanes)) pushStoreConsistencyCheck(fiber, getSnapshot, nextSnapshot);
			}
		...
		function forceStoreRerender(fiber) {
			...
			if (root2 !== null) scheduleUpdateOnFiber(root2, fiber, SyncLane, NoTimestamp);
		}
		...
		function dispatchReducerAction(fiber, queue, action) {
			...
			if (isRenderPhaseUpdate(fiber)) enqueueRenderPhaseUpdate(queue, update);
				...
				if (root2 !== null) {
					scheduleUpdateOnFiber(root2, fiber, lane, requestEventTime());
			...
			if (passiveEffectStartTime >= 0) {
				...
				while (parentFiber !== null) {
					switch (parentFiber.tag) {
						case HostRoot:
							var root2 = parentFiber.stateNode;
							if (root2 !== null) root2.passiveEffectDuration += elapsedTime;
							return;
						case Profiler:
							var parentStateNode = parentFiber.stateNode;
		...
		function constructClassInstance(workInProgress2, ctor, props) {
			...
			if (isLegacyContextConsumer) cacheContext(workInProgress2, unmaskedContext, context);
			return instance;
		}
		...
		function mountClassInstance(workInProgress2, ctor, newProps, renderLanes2) {
			...
			else instance.context = getMaskedContext(workInProgress2, getUnmaskedContext(workInProgress2, ctor, true));
			...
			if (workInProgress2.mode & StrictLegacyMode) ReactStrictModeWarnings.recordLegacyContextWarning(workInProgress2, instance);
			...
			if (typeof instance.componentDidMount === "function") {
				var fiberFlags = Update;
				fiberFlags |= LayoutStatic;
				if ((workInProgress2.mode & StrictEffectsMode) !== NoMode) fiberFlags |= MountLayoutDev;
		...
		function resumeMountClassInstance(workInProgress2, ctor, newProps, renderLanes2) {
			...
			else nextContext = getMaskedContext(workInProgress2, getUnmaskedContext(workInProgress2, ctor, true));
			var getDerivedStateFromProps = ctor.getDerivedStateFromProps;
			var hasNewLifecycles = typeof getDerivedStateFromProps === "function" || typeof instance.getSnapshotBeforeUpdate === "function";
			if (!hasNewLifecycles && (typeof instance.UNSAFE_componentWillReceiveProps === "function" || typeof instance.componentWillReceiveProps === "function")) {
				if (oldProps !== newProps || oldContext !== nextContext) callComponentWillReceiveProps(workInProgress2, instance, newProps, nextContext);
			}
			...
			if (shouldUpdate) {
				...
				if (typeof instance.componentDidMount === "function") {
					...
					if ((workInProgress2.mode & StrictEffectsMode) !== NoMode) _fiberFlags |= MountLayoutDev;
				...
				if (typeof instance.componentDidMount === "function") {
					...
					if ((workInProgress2.mode & StrictEffectsMode) !== NoMode) _fiberFlags2 |= MountLayoutDev;
		...
		function updateClassInstance(current2, workInProgress2, ctor, newProps, renderLanes2) {
			...
			var hasNewLifecycles = typeof getDerivedStateFromProps === "function" || typeof instance.getSnapshotBeforeUpdate === "function";
			if (!hasNewLifecycles && (typeof instance.UNSAFE_componentWillReceiveProps === "function" || typeof instance.componentWillReceiveProps === "function")) {
				if (unresolvedOldProps !== unresolvedNewProps || oldContext !== nextContext) callComponentWillReceiveProps(workInProgress2, instance, newProps, nextContext);
			}
			resetHasForceUpdateBeforeProcessing();
			var oldState = workInProgress2.memoizedState;
			var newState = instance.state = oldState;
			...
			if (unresolvedOldProps === unresolvedNewProps && oldState === newState && !hasContextChanged() && !checkHasForceUpdateAfterProcessing() && !enableLazyContextPropagation) {
				if (typeof instance.componentDidUpdate === "function") {
					if (unresolvedOldProps !== current2.memoizedProps || oldState !== current2.memoizedState) workInProgress2.flags |= Update;
				}
				if (typeof instance.getSnapshotBeforeUpdate === "function") {
					if (unresolvedOldProps !== current2.memoizedProps || oldState !== current2.memoizedState) workInProgress2.flags |= Snapshot;
				}
				return false;
			}
			if (typeof getDerivedStateFromProps === "function") {
			...
			if (shouldUpdate) {
				...
				if (typeof instance.componentDidUpdate === "function") workInProgress2.flags |= Update;
				if (typeof instance.getSnapshotBeforeUpdate === "function") workInProgress2.flags |= Snapshot;
			} else {
				if (typeof instance.componentDidUpdate === "function") {
					if (unresolvedOldProps !== current2.memoizedProps || oldState !== current2.memoizedState) workInProgress2.flags |= Update;
				}
		...
		function attachPingListener(root2, wakeable, lanes) {
			...
			if (!threadIDs.has(lanes)) {
				...
				if (isDevToolsPresent) restorePendingUpdaters(root2, lanes);
		...
		function throwException(root2, returnFiber, sourceFiber, value, rootRenderLanes) {
			...
			if (isDevToolsPresent) restorePendingUpdaters(root2, rootRenderLanes);
			if (value !== null && typeof value === "object" && typeof value.then === "function") {
				...
				if (suspenseBoundary !== null) {
					...
					if (suspenseBoundary.mode & ConcurrentMode) attachPingListener(root2, wakeable, rootRenderLanes);
		...
		function reconcileChildren(current2, workInProgress2, nextChildren, renderLanes2) {
			if (current2 === null) workInProgress2.child = mountChildFibers(workInProgress2, null, nextChildren, renderLanes2);
			else workInProgress2.child = reconcileChildFibers(workInProgress2, current2.child, nextChildren, renderLanes2);
		}
		...
		function updateForwardRef(current2, workInProgress2, Component, nextProps, renderLanes2) {
			...
			if (getIsHydrating() && hasId) pushMaterializedTreeId(workInProgress2);
		...
		function updateMemoComponent(current2, workInProgress2, Component, nextProps, renderLanes2) {
			...
			if (!checkScheduledUpdateOrContext(current2, renderLanes2)) {
				...
				if (compare(prevProps, nextProps) && current2.ref === workInProgress2.ref) return bailoutOnAlreadyFinishedWork(current2, workInProgress2, renderLanes2);
			}
			workInProgress2.flags |= PerformedWork;
			...
			if (current2 !== null) {
				...
				if (shallowEqual(prevProps, nextProps) && current2.ref === workInProgress2.ref && workInProgress2.type === current2.type) {
					...
					if (!checkScheduledUpdateOrContext(current2, renderLanes2)) {
						...
						return bailoutOnAlreadyFinishedWork(current2, workInProgress2, renderLanes2);
					} else if ((current2.flags & ForceUpdateForLegacySuspense) !== NoFlags) didReceiveUpdate = true;
				}
			}
		...
		function updateOffscreenComponent(current2, workInProgress2, renderLanes2) {
			...
			if (nextProps.mode === "hidden" || enableLegacyHidden) if ((workInProgress2.mode & ConcurrentMode) === NoMode) {
				...
				pushRenderLanes(workInProgress2, prevState !== null ? prevState.baseLanes : renderLanes2);
			}
			else {
				...
				if (prevState !== null) {
					...
					workInProgress2.memoizedState = null;
				} else _subtreeRenderLanes = renderLanes2;
		...
		function updateFunctionComponent(current2, workInProgress2, Component, nextProps, renderLanes2) {
			...
			var context = getMaskedContext(workInProgress2, getUnmaskedContext(workInProgress2, Component, true));
			var nextChildren;
			var hasId;
			prepareToReadContext(workInProgress2, renderLanes2);
			markComponentRenderStarted(workInProgress2);
		...
		function updateClassComponent(current2, workInProgress2, Component, nextProps, renderLanes2) {
			switch (shouldError(workInProgress2)) {
				case false:
					...
					var ctor = workInProgress2.type;
					var state = new ctor(workInProgress2.memoizedProps, _instance.context).state;
				...
				case true:
					...
					workInProgress2.lanes = mergeLanes(workInProgress2.lanes, lane);
					enqueueCapturedUpdate(workInProgress2, createClassErrorUpdate(workInProgress2, createCapturedValueAtFiber(error$1, workInProgress2), lane));
					break;
			}
			if (workInProgress2.type !== workInProgress2.elementType) {
				var innerPropTypes = Component.propTypes;
			...
			if (instance === null) {
				...
				shouldUpdate = true;
			} else if (current2 === null) shouldUpdate = resumeMountClassInstance(workInProgress2, Component, nextProps, renderLanes2);
			else shouldUpdate = updateClassInstance(current2, workInProgress2, Component, nextProps, renderLanes2);
			var nextUnitOfWork = finishClassComponent(current2, workInProgress2, Component, shouldUpdate, hasContext, renderLanes2);
			var inst = workInProgress2.stateNode;
			if (shouldUpdate && inst.props !== nextProps) {
				if (!didWarnAboutReassigningProps) error("It looks like %s is reassigning its own `this.props` while rendering. This is not supported and can lead to confusing bugs.", getComponentNameFromFiber(workInProgress2) || "a component");
		...
		function finishClassComponent(current2, workInProgress2, Component, shouldUpdate, hasContext, renderLanes2) {
			...
			if (!shouldUpdate && !didCaptureError) {
				if (hasContext) invalidateContextProvider(workInProgress2, Component, false);
				return bailoutOnAlreadyFinishedWork(current2, workInProgress2, renderLanes2);
			}
			var instance = workInProgress2.stateNode;
			ReactCurrentOwner$1.current = workInProgress2;
			...
			workInProgress2.flags |= PerformedWork;
			if (current2 !== null && didCaptureError) forceUnmountCurrentAndReconcile(current2, workInProgress2, nextChildren, renderLanes2);
			else reconcileChildren(current2, workInProgress2, nextChildren, renderLanes2);
			workInProgress2.memoizedState = instance.state;
			if (hasContext) invalidateContextProvider(workInProgress2, Component, true);
			return workInProgress2.child;
		}
		function pushHostRootContext(workInProgress2) {
			var root2 = workInProgress2.stateNode;
			if (root2.pendingContext) pushTopLevelContextObject(workInProgress2, root2.pendingContext, root2.pendingContext !== root2.context);
			else if (root2.context) pushTopLevelContextObject(workInProgress2, root2.context, false);
			pushHostContainer(workInProgress2, root2.containerInfo);
		}
		function updateHostRoot(current2, workInProgress2, renderLanes2) {
			pushHostRootContext(workInProgress2);
			if (current2 === null) throw new Error("Should have a current fiber. This is a bug in React.");
			var nextProps = workInProgress2.pendingProps;
			var prevState = workInProgress2.memoizedState;
			...
			if (prevState.isDehydrated) {
				...
				if (workInProgress2.flags & ForceClientRender) return mountHostRootWithoutHydrating(current2, workInProgress2, nextChildren, renderLanes2, createCapturedValueAtFiber(/* @__PURE__ */ new Error("There was an error while hydrating. Because the error happened outside of a Suspense boundary, the entire root will switch to client rendering."), workInProgress2));
				else if (nextChildren !== prevChildren) return mountHostRootWithoutHydrating(current2, workInProgress2, nextChildren, renderLanes2, createCapturedValueAtFiber(/* @__PURE__ */ new Error("This root received an early update, before anything was able hydrate. Switched the entire root to client rendering."), workInProgress2));
				else {
				...
				resetHydrationState();
				if (nextChildren === prevChildren) return bailoutOnAlreadyFinishedWork(current2, workInProgress2, renderLanes2);
				reconcileChildren(current2, workInProgress2, nextChildren, renderLanes2);
			}
			return workInProgress2.child;
		}
		...
		function updateHostComponent(current2, workInProgress2, renderLanes2) {
			...
			if (current2 === null) tryToClaimNextHydratableInstance(workInProgress2);
			var type = workInProgress2.type;
			var nextProps = workInProgress2.pendingProps;
			var prevProps = current2 !== null ? current2.memoizedProps : null;
			var nextChildren = nextProps.children;
			if (shouldSetTextContent(type, nextProps)) nextChildren = null;
			else if (prevProps !== null && shouldSetTextContent(type, prevProps)) workInProgress2.flags |= ContentReset;
			markRef(current2, workInProgress2);
			reconcileChildren(current2, workInProgress2, nextChildren, renderLanes2);
			return workInProgress2.child;
		}
		...
		function mountIndeterminateComponent(_current, workInProgress2, Component, renderLanes2) {
			resetSuspendedCurrentOnMountInLegacyMode(_current, workInProgress2);
			var props = workInProgress2.pendingProps;
			var context = getMaskedContext(workInProgress2, getUnmaskedContext(workInProgress2, Component, false));
			prepareToReadContext(workInProgress2, renderLanes2);
			var value;
			var hasId;
			markComponentRenderStarted(workInProgress2);
			...
			if (workInProgress2.mode & StrictLegacyMode) ReactStrictModeWarnings.recordLegacyContextWarning(workInProgress2, null);
			setIsRendering(true);
			ReactCurrentOwner$1.current = workInProgress2;
		...
		function shouldRemainOnFallback(suspenseContext, current2, workInProgress2, renderLanes2) {
			if (current2 !== null) {
				if (current2.memoizedState === null) return false;
			}
		...
		function updateSuspenseComponent(current2, workInProgress2, renderLanes2) {
			var nextProps = workInProgress2.pendingProps;
			if (shouldSuspend(workInProgress2)) workInProgress2.flags |= DidCapture;
			...
			if (didSuspend || shouldRemainOnFallback(suspenseContext, current2)) {
				showFallback = true;
				workInProgress2.flags &= ~DidCapture;
			} else if (current2 === null || current2.memoizedState !== null) suspenseContext = addSubtreeSuspenseContext(suspenseContext, InvisibleParentSuspenseContext);
			...
			if (current2 === null) {
				...
				if (suspenseState !== null) {
					...
					if (dehydrated !== null) return mountDehydratedSuspenseComponent(workInProgress2, dehydrated);
				}
				...
				} else return mountSuspensePrimaryChildren(workInProgress2, nextPrimaryChildren);
			} else {
				var prevState = current2.memoizedState;
				if (prevState !== null) {
					var _dehydrated = prevState.dehydrated;
					if (_dehydrated !== null) return updateDehydratedSuspenseComponent(current2, workInProgress2, didSuspend, nextProps, _dehydrated, prevState, renderLanes2);
				}
				if (showFallback) {
		...
		function updateSuspensePrimaryChildren(current2, workInProgress2, primaryChildren, renderLanes2) {
			...
			if ((workInProgress2.mode & ConcurrentMode) === NoMode) primaryChildFragment.lanes = renderLanes2;
			primaryChildFragment.return = workInProgress2;
		...
		function retrySuspenseComponentWithoutHydrating(current2, workInProgress2, renderLanes2, recoverableError) {
			...
			var primaryChildren = workInProgress2.pendingProps.children;
		...
		function mountSuspenseFallbackAfterRetryWithoutHydrating(current2, workInProgress2, primaryChildren, fallbackChildren, renderLanes2) {
			...
			primaryChildFragment.sibling = fallbackChildFragment;
			workInProgress2.child = primaryChildFragment;
			if ((workInProgress2.mode & ConcurrentMode) !== NoMode) reconcileChildFibers(workInProgress2, current2.child, null, renderLanes2);
			return fallbackChildFragment;
		}
		function mountDehydratedSuspenseComponent(workInProgress2, suspenseInstance, renderLanes2) {
			...
			} else if (isSuspenseInstanceFallback(suspenseInstance)) workInProgress2.lanes = laneToLanes(DefaultHydrationLane);
			else workInProgress2.lanes = laneToLanes(OffscreenLane);
			return null;
		}
		function updateDehydratedSuspenseComponent(current2, workInProgress2, didSuspend, nextProps, suspenseInstance, suspenseState, renderLanes2) {
			if (!didSuspend) {
				warnIfHydrating();
				if ((workInProgress2.mode & ConcurrentMode) === NoMode) return retrySuspenseComponentWithoutHydrating(current2, workInProgress2, renderLanes2, null);
				if (isSuspenseInstanceFallback(suspenseInstance)) {
					...
					var error2;
					if (message) error2 = new Error(message);
					else error2 = /* @__PURE__ */ new Error("The server could not finish this Suspense boundary, likely due to an error during server rendering. Switched to client rendering.");
					return retrySuspenseComponentWithoutHydrating(current2, workInProgress2, renderLanes2, createCapturedValue(error2, digest, stack));
				}
				...
				if (didReceiveUpdate || hasContextChanged2) {
					...
					renderDidSuspendDelayIfPossible();
					return retrySuspenseComponentWithoutHydrating(current2, workInProgress2, renderLanes2, createCapturedValue(/* @__PURE__ */ new Error("This Suspense boundary received an update before it finished hydrating. This caused the boundary to switch to client rendering. The usual way to fix this is to wrap the original update in startTransition.")));
				} else if (isSuspenseInstancePending(suspenseInstance)) {
					workInProgress2.flags |= DidCapture;
					workInProgress2.child = current2.child;
					registerSuspenseInstanceRetry(suspenseInstance, retryDehydratedSuspenseBoundary.bind(null, current2));
					return null;
				} else {
				...
				workInProgress2.flags &= ~ForceClientRender;
				return retrySuspenseComponentWithoutHydrating(current2, workInProgress2, renderLanes2, createCapturedValue(/* @__PURE__ */ new Error("There was an error while hydrating this Suspense boundary. Switched to client rendering.")));
		...
		function propagateSuspenseContextChange(workInProgress2, firstChild, renderLanes2) {
			...
			while (node !== null) {
				if (node.tag === SuspenseComponent) {
					if (node.memoizedState !== null) scheduleSuspenseWorkOnFiber(node, renderLanes2, workInProgress2);
				} else if (node.tag === SuspenseListComponent) scheduleSuspenseWorkOnFiber(node, renderLanes2, workInProgress2);
				else if (node.child !== null) {
					node.child.return = node;
					node = node.child;
					continue;
		...
		function initSuspenseListRenderState(workInProgress2, isBackwards, tail, lastContentRow, tailMode) {
			...
			if (renderState === null) workInProgress2.memoizedState = {
				isBackwards,
				rendering: null,
		...
		function updateSuspenseListComponent(current2, workInProgress2, renderLanes2) {
			...
			if (hasSuspenseContext(suspenseContext, ForceSuspenseFallback)) {
				...
				if (current2 !== null && (current2.flags & DidCapture) !== NoFlags) propagateSuspenseContextChange(workInProgress2, workInProgress2.child, renderLanes2);
				suspenseContext = setDefaultShallowSuspenseContext(suspenseContext);
			}
			pushSuspenseContext(workInProgress2, suspenseContext);
			if ((workInProgress2.mode & ConcurrentMode) === NoMode) workInProgress2.memoizedState = null;
				...
				case "together":
					...
					break;
				default: workInProgress2.memoizedState = null;
			}
			return workInProgress2.child;
		}
		function updatePortalComponent(current2, workInProgress2, renderLanes2) {
			...
			var nextChildren = workInProgress2.pendingProps;
			if (current2 === null) workInProgress2.child = reconcileChildFibers(workInProgress2, null, nextChildren, renderLanes2);
			else reconcileChildren(current2, workInProgress2, nextChildren, renderLanes2);
			return workInProgress2.child;
		}
		...
		function updateContextProvider(current2, workInProgress2, renderLanes2) {
			...
			if (oldProps !== null) {
				...
				if (objectIs(oldValue, newValue)) {
					if (oldProps.children === newProps.children && !hasContextChanged()) return bailoutOnAlreadyFinishedWork(current2, workInProgress2, renderLanes2);
				} else propagateContextChange(workInProgress2, context, renderLanes2);
			}
		...
		function updateContextConsumer(current2, workInProgress2, renderLanes2) {
			...
			var render2 = workInProgress2.pendingProps.children;
			if (typeof render2 !== "function") error("A context consumer was rendered with multiple children, or a child that isn't a function. A context consumer expects a single child that is a function. If you did pass a function, make sure there is no trailing or leading whitespace around it.");
			prepareToReadContext(workInProgress2, renderLanes2);
		...
		function bailoutOnAlreadyFinishedWork(current2, workInProgress2, renderLanes2) {
			if (current2 !== null) workInProgress2.dependencies = current2.dependencies;
			stopProfilerTimerIfRunning();
			markSkippedUpdateLanes(workInProgress2.lanes);
			if (!includesSomeLane(renderLanes2, workInProgress2.childLanes)) return null;
		...
		function remountFiber(current2, oldWorkInProgress, newWorkInProgress) {
			...
			if (deletions === null) {
				...
				returnFiber.flags |= ChildDeletion;
			} else deletions.push(current2);
			newWorkInProgress.flags |= Placement;
			return newWorkInProgress;
		}
		function checkScheduledUpdateOrContext(current2, renderLanes2) {
			var updateLanes = current2.lanes;
			if (includesSomeLane(updateLanes, renderLanes2)) return true;
			return false;
		}
		function attemptEarlyBailoutIfNoScheduledUpdate(current2, workInProgress2, renderLanes2) {
			switch (workInProgress2.tag) {
				...
				case ClassComponent:
					...
					if (isContextProvider(Component)) pushContextProvider(workInProgress2);
					break;
				case HostPortal:
					pushHostContainer(workInProgress2, workInProgress2.stateNode.containerInfo);
					break;
				case ContextProvider:
					...
					break;
				case Profiler:
					if (includesSomeLane(renderLanes2, workInProgress2.childLanes)) workInProgress2.flags |= Update;
					var stateNode = workInProgress2.stateNode;
					stateNode.effectDuration = 0;
					stateNode.passiveEffectDuration = 0;
					break;
				case SuspenseComponent:
					...
					if (state !== null) {
						...
						if (includesSomeLane(renderLanes2, primaryChildLanes)) return updateSuspenseComponent(current2, workInProgress2, renderLanes2);
						else {
							pushSuspenseContext(workInProgress2, setDefaultShallowSuspenseContext(suspenseStackCursor.current));
					...
					} else pushSuspenseContext(workInProgress2, setDefaultShallowSuspenseContext(suspenseStackCursor.current));
					break;
				case SuspenseListComponent:
					var didSuspendBefore = (current2.flags & DidCapture) !== NoFlags;
					var _hasChildWork = includesSomeLane(renderLanes2, workInProgress2.childLanes);
					if (didSuspendBefore) {
						if (_hasChildWork) return updateSuspenseListComponent(current2, workInProgress2, renderLanes2);
						workInProgress2.flags |= DidCapture;
					}
					var renderState = workInProgress2.memoizedState;
					if (renderState !== null) {
		...
		function beginWork(current2, workInProgress2, renderLanes2) {
			if (workInProgress2._debugNeedsRemount && current2 !== null) return remountFiber(current2, workInProgress2, createFiberFromTypeAndProps(workInProgress2.type, workInProgress2.key, workInProgress2.pendingProps, workInProgress2._debugOwner || null, workInProgress2.mode, workInProgress2.lanes));
			if (current2 !== null) if (current2.memoizedProps !== workInProgress2.pendingProps || hasContextChanged() || workInProgress2.type !== current2.type) didReceiveUpdate = true;
			else {
				if (!checkScheduledUpdateOrContext(current2, renderLanes2) && (workInProgress2.flags & DidCapture) === NoFlags) {
					didReceiveUpdate = false;
					return attemptEarlyBailoutIfNoScheduledUpdate(current2, workInProgress2, renderLanes2);
				}
				if ((current2.flags & ForceUpdateForLegacySuspense) !== NoFlags) didReceiveUpdate = true;
				...
				if (getIsHydrating() && isForkedChild(workInProgress2)) {
					...
					pushTreeId(workInProgress2, getForksAtLevel(), slotIndex);
				}
			}
			workInProgress2.lanes = NoLanes;
			switch (workInProgress2.tag) {
				case IndeterminateComponent: return mountIndeterminateComponent(current2, workInProgress2, workInProgress2.type, renderLanes2);
				case LazyComponent:
				...
				case FunctionComponent:
					...
					return updateFunctionComponent(current2, workInProgress2, Component, workInProgress2.elementType === Component ? unresolvedProps : resolveDefaultProps(Component, unresolvedProps), renderLanes2);
				case ClassComponent:
					var _Component = workInProgress2.type;
					var _unresolvedProps = workInProgress2.pendingProps;
					return updateClassComponent(current2, workInProgress2, _Component, workInProgress2.elementType === _Component ? _unresolvedProps : resolveDefaultProps(_Component, _unresolvedProps), renderLanes2);
				case HostRoot: return updateHostRoot(current2, workInProgress2, renderLanes2);
				case HostComponent: return updateHostComponent(current2, workInProgress2, renderLanes2);
				case HostText: return updateHostText(current2, workInProgress2);
				case SuspenseComponent: return updateSuspenseComponent(current2, workInProgress2, renderLanes2);
				case HostPortal: return updatePortalComponent(current2, workInProgress2, renderLanes2);
				case ForwardRef:
					var type = workInProgress2.type;
					var _unresolvedProps2 = workInProgress2.pendingProps;
					return updateForwardRef(current2, workInProgress2, type, workInProgress2.elementType === type ? _unresolvedProps2 : resolveDefaultProps(type, _unresolvedProps2), renderLanes2);
				case Fragment: return updateFragment(current2, workInProgress2, renderLanes2);
				case Mode: return updateMode(current2, workInProgress2, renderLanes2);
				case Profiler: return updateProfiler(current2, workInProgress2, renderLanes2);
				case ContextProvider: return updateContextProvider(current2, workInProgress2, renderLanes2);
				case ContextConsumer: return updateContextConsumer(current2, workInProgress2, renderLanes2);
				case MemoComponent:
				...
				case SimpleMemoComponent: return updateSimpleMemoComponent(current2, workInProgress2, workInProgress2.type, workInProgress2.pendingProps, renderLanes2);
				case IncompleteClassComponent:
					var _Component2 = workInProgress2.type;
					var _unresolvedProps4 = workInProgress2.pendingProps;
					return mountIncompleteClassComponent(current2, workInProgress2, _Component2, workInProgress2.elementType === _Component2 ? _unresolvedProps4 : resolveDefaultProps(_Component2, _unresolvedProps4), renderLanes2);
				case SuspenseListComponent: return updateSuspenseListComponent(current2, workInProgress2, renderLanes2);
				case ScopeComponent: break;
				case OffscreenComponent: return updateOffscreenComponent(current2, workInProgress2, renderLanes2);
			}
		...
		updateHostComponent$1 = function(current2, workInProgress2, type, newProps, rootContainerInstance) {
			...
			if (updatePayload) markUpdate(workInProgress2);
		};
		updateHostText$1 = function(current2, workInProgress2, oldText, newText) {
			if (oldText !== newText) markUpdate(workInProgress2);
		};
		...
		function completeDehydratedSuspenseBoundary(current2, workInProgress2, nextState) {
			...
			if (nextState !== null && nextState.dehydrated !== null) if (current2 === null) {
				...
				if ((workInProgress2.mode & ProfileMode) !== NoMode) {
					if (nextState !== null) {
						var primaryChildFragment = workInProgress2.child;
						if (primaryChildFragment !== null) workInProgress2.treeBaseDuration -= primaryChildFragment.treeBaseDuration;
					}
				}
				return false;
			} else {
				resetHydrationState();
				if ((workInProgress2.flags & DidCapture) === NoFlags) workInProgress2.memoizedState = null;
				workInProgress2.flags |= Update;
				bubbleProperties(workInProgress2);
				if ((workInProgress2.mode & ProfileMode) !== NoMode) {
					if (nextState !== null) {
						var _primaryChildFragment = workInProgress2.child;
						if (_primaryChildFragment !== null) workInProgress2.treeBaseDuration -= _primaryChildFragment.treeBaseDuration;
					}
				}
				return false;
			}
		...
		function completeWork(current2, workInProgress2, renderLanes2) {
			...
			switch (workInProgress2.tag) {
				...
				case ClassComponent:
					var Component = workInProgress2.type;
					if (isContextProvider(Component)) popContext(workInProgress2);
					bubbleProperties(workInProgress2);
					return null;
				case HostRoot:
					...
					if (current2 === null || current2.child === null) {
						if (popHydrationState(workInProgress2)) markUpdate(workInProgress2);
						else if (current2 !== null) {
							if (!current2.memoizedState.isDehydrated || (workInProgress2.flags & ForceClientRender) !== NoFlags) {
				...
				case HostComponent:
					...
					if (current2 !== null && workInProgress2.stateNode != null) {
						...
						if (current2.ref !== workInProgress2.ref) markRef$1(workInProgress2);
					} else {
						if (!newProps) {
							if (workInProgress2.stateNode === null) throw new Error("We must have new props for new mounts. This error is likely caused by a bug in React. Please file an issue.");
							bubbleProperties(workInProgress2);
							return null;
						}
						var currentHostContext = getHostContext();
						if (popHydrationState(workInProgress2)) {
							if (prepareToHydrateHostInstance(workInProgress2, rootContainerInstance, currentHostContext)) markUpdate(workInProgress2);
						} else {
							var instance = createInstance(type, newProps, rootContainerInstance, currentHostContext, workInProgress2);
							appendAllChildren(instance, workInProgress2, false, false);
							workInProgress2.stateNode = instance;
							if (finalizeInitialChildren(instance, type, newProps, rootContainerInstance)) markUpdate(workInProgress2);
						}
						if (workInProgress2.ref !== null) markRef$1(workInProgress2);
					}
					bubbleProperties(workInProgress2);
					return null;
				case HostText:
					...
					if (current2 && workInProgress2.stateNode != null) {
						...
						if (popHydrationState(workInProgress2)) {
							if (prepareToHydrateHostTextInstance(workInProgress2)) markUpdate(workInProgress2);
						} else workInProgress2.stateNode = createTextInstance(newText, _rootContainerInstance, _currentHostContext, workInProgress2);
					}
					bubbleProperties(workInProgress2);
					return null;
				case SuspenseComponent:
					...
					if (current2 === null || current2.memoizedState !== null && current2.memoizedState.dehydrated !== null) {
						if (!completeDehydratedSuspenseBoundary(current2, workInProgress2, nextState)) if (workInProgress2.flags & ShouldCapture) return workInProgress2;
						else return null;
					}
					...
					if (nextDidTimeout !== (current2 !== null && current2.memoizedState !== null)) {
						if (nextDidTimeout) {
							var _offscreenFiber2 = workInProgress2.child;
							_offscreenFiber2.flags |= Visibility;
							if ((workInProgress2.mode & ConcurrentMode) !== NoMode) if (current2 === null && (workInProgress2.memoizedProps.unstable_avoidThisFallback !== true || !enableSuspenseAvoidThisFallback) || hasSuspenseContext(suspenseStackCursor.current, InvisibleParentSuspenseContext)) renderDidSuspend();
							else renderDidSuspendDelayIfPossible();
						}
					}
					if (workInProgress2.updateQueue !== null) workInProgress2.flags |= Update;
					bubbleProperties(workInProgress2);
					if ((workInProgress2.mode & ProfileMode) !== NoMode) {
						if (nextDidTimeout) {
							var primaryChildFragment = workInProgress2.child;
				...
				case HostPortal:
					popHostContainer(workInProgress2);
					updateHostContainer(current2, workInProgress2);
					if (current2 === null) preparePortalMount(workInProgress2.stateNode.containerInfo);
					bubbleProperties(workInProgress2);
					return null;
				case ContextProvider:
					var context = workInProgress2.type._context;
				...
				case IncompleteClassComponent:
					var _Component = workInProgress2.type;
					if (isContextProvider(_Component)) popContext(workInProgress2);
					bubbleProperties(workInProgress2);
					return null;
				case SuspenseListComponent:
					popSuspenseContext(workInProgress2);
					...
					if (renderedTail === null) if (!didSuspendAlready) {
						if (!(renderHasNotSuspendedYet() && (current2 === null || (current2.flags & DidCapture) === NoFlags))) {
							var row = workInProgress2.child;
							while (row !== null) {
						...
						if (renderState.isBackwards) {
							...
							else workInProgress2.child = renderedTail;
				...
				case LegacyHiddenComponent:
					...
					var nextIsHidden = workInProgress2.memoizedState !== null;
					if (current2 !== null) {
						if (current2.memoizedState !== null !== nextIsHidden && !enableLegacyHidden) workInProgress2.flags |= Visibility;
					}
					if (!nextIsHidden || (workInProgress2.mode & ConcurrentMode) === NoMode) bubbleProperties(workInProgress2);
					else if (includesSomeLane(subtreeRenderLanes, OffscreenLane)) {
						bubbleProperties(workInProgress2);
						if (workInProgress2.subtreeFlags & (Placement | Update)) workInProgress2.flags |= Visibility;
					}
					return null;
			...
			switch (workInProgress2.tag) {
				...
				case SuspenseComponent:
					...
					if (suspenseState !== null && suspenseState.dehydrated !== null) {
						if (workInProgress2.alternate === null) throw new Error("Threw in newly mounted dehydrated component. This is likely a bug in React. Please file an issue.");
						resetHydrationState();
					}
		...
		var callComponentWillUnmountWithTimer = function(current2, instance) {
			...
			if (current2.mode & ProfileMode) try {
		...
		function safelyDetachRef(current2, nearestMountedAncestor) {
			...
			if (ref !== null) if (typeof ref === "function") {
				var retVal;
				try {
					if (enableProfilerTimer && enableProfilerCommitHooks && current2.mode & ProfileMode) try {
						startLayoutEffectTimer();
						retVal = ref(null);
					} finally {
						recordLayoutEffectDuration(current2);
				...
				if (typeof retVal === "function") error("Unexpected return value from a callback ref in %s. A callback ref should not return a function.", getComponentNameFromFiber(current2));
		...
		function commitPassiveEffectDurations(finishedRoot, finishedWork) {
			if ((finishedWork.flags & Update) !== NoFlags) switch (finishedWork.tag) {
				case Profiler:
					...
					if (typeof onPostCommit === "function") onPostCommit(id, phase, passiveEffectDuration, commitTime2);
			...
			if ((finishedWork.flags & LayoutMask) !== NoFlags) switch (finishedWork.tag) {
				...
				case ClassComponent:
					...
					if (finishedWork.flags & Update) {
						if (!offscreenSubtreeWasHidden) if (current2 === null) {
							if (finishedWork.type === finishedWork.elementType && !didWarnAboutReassigningProps) {
				...
				case Profiler:
					...
					if (isCurrentUpdateNested()) phase = "nested-update";
					if (typeof onRender === "function") onRender(finishedWork.memoizedProps.id, phase, finishedWork.actualDuration, finishedWork.treeBaseDuration, finishedWork.actualStartTime, commitTime2);
					if (typeof onCommit === "function") onCommit(finishedWork.memoizedProps.id, phase, effectDuration, commitTime2);
		...
		function commitMutationEffectsOnFiber(finishedWork, root2, lanes) {
			...
			switch (finishedWork.tag) {
				...
				case ClassComponent:
					...
					commitReconciliationEffects(finishedWork);
					if (flags & Ref) {
						if (current2 !== null) safelyDetachRef(current2, current2.return);
					}
					return;
				case HostComponent:
					recursivelyTraverseMutationEffects(root2, finishedWork);
				...
				case HostRoot:
					...
					if (flags & Update) {
						if (current2 !== null) {
							if (current2.memoizedState.isDehydrated) try {
				...
				case OffscreenComponent:
					...
					if (finishedWork.mode & ConcurrentMode) {
						...
						recursivelyTraverseMutationEffects(root2, finishedWork);
						offscreenSubtreeWasHidden = prevOffscreenSubtreeWasHidden;
					} else recursivelyTraverseMutationEffects(root2, finishedWork);
					commitReconciliationEffects(finishedWork);
					if (flags & Visibility) {
		...
		function commitLayoutEffects_begin(subtreeRoot, root2, committedLanes) {
			...
			while (nextEffect !== null) {
				...
				if ((fiber.subtreeFlags & LayoutMask) !== NoFlags && firstChild !== null) {
					...
					nextEffect = firstChild;
				} else commitLayoutMountEffects_complete(subtreeRoot, root2, committedLanes);
			}
		}
		...
		function commitPassiveMountEffects_begin(subtreeRoot, root2, committedLanes, committedTransitions) {
			while (nextEffect !== null) {
				...
				if ((fiber.subtreeFlags & PassiveMask) !== NoFlags && firstChild !== null) {
					firstChild.return = fiber;
					nextEffect = firstChild;
				} else commitPassiveMountEffects_complete(subtreeRoot, root2, committedLanes, committedTransitions);
			}
		}
		...
		function commitPassiveUnmountInsideDeletedTreeOnFiber(current2, nearestMountedAncestor) {
			switch (current2.tag) {
				...
				case SimpleMemoComponent:
					...
					} else commitHookEffectListUnmount(Passive$1, current2, nearestMountedAncestor);
					break;
			}
		}
		...
		function scheduleUpdateOnFiber(root2, fiber, lane, eventTime) {
			...
			if ((executionContext & RenderContext) !== NoLanes && root2 === workInProgressRoot) warnAboutRenderPhaseUpdatesInDEV(fiber);
			else {
				if (isDevToolsPresent) addFiberToLanesMap(root2, fiber, lane);
				...
				if (root2 === workInProgressRoot) {
					...
					if (workInProgressRootExitStatus === RootSuspendedWithDelay) markRootSuspended$1(root2, workInProgressRootRenderLanes);
				}
		...
		function ensureRootIsScheduled(root2, currentTime) {
			...
			if (newCallbackPriority === SyncLane) {
				...
				} else scheduleSyncCallback(performSyncWorkOnRoot.bind(null, root2));
		...
		function performConcurrentWorkOnRoot(root2, didTimeout) {
			...
			if (flushPassiveEffects()) {
				if (root2.callbackNode !== originalCallbackNode) return null;
			}
			var lanes = getNextLanes(root2, root2 === workInProgressRoot ? workInProgressRootRenderLanes : NoLanes);
			if (lanes === NoLanes) return null;
			var exitStatus = !includesBlockingLane(root2, lanes) && !includesExpiredLane(root2, lanes) && !didTimeout ? renderRootConcurrent(root2, lanes) : renderRootSync(root2, lanes);
			if (exitStatus !== RootInProgress) {
				...
				if (exitStatus === RootDidNotComplete) markRootSuspended$1(root2, lanes);
				else {
			...
			ensureRootIsScheduled(root2, now());
			if (root2.callbackNode === originalCallbackNode) return performConcurrentWorkOnRoot.bind(null, root2);
			return null;
		}
			...
			switch (exitStatus) {
				...
				case RootSuspended:
					...
					if (includesOnlyRetries(lanes) && !shouldForceFlushFallbacksInDEV()) {
						...
						if (msUntilTimeout > 10) {
							if (getNextLanes(root2, NoLanes) !== NoLanes) break;
							var suspendedLanes = root2.suspendedLanes;
				...
				case RootSuspendedWithDelay:
					...
					if (!shouldForceFlushFallbacksInDEV()) {
						var eventTimeMs = getMostRecentEventTime(root2, lanes);
		...
		function renderDidError(error2) {
			...
			if (workInProgressRootConcurrentErrors === null) workInProgressRootConcurrentErrors = [error2];
			else workInProgressRootConcurrentErrors.push(error2);
		}
		...
		function performUnitOfWork(unitOfWork) {
			...
			} else next = beginWork$1(current2, unitOfWork, subtreeRenderLanes);
			resetCurrentFiber();
		...
		function completeUnitOfWork(unitOfWork) {
			...
			do {
				...
				if ((completedWork.flags & Incomplete) === NoFlags) {
					...
					var next = void 0;
					if ((completedWork.mode & ProfileMode) === NoMode) next = completeWork(current2, completedWork, subtreeRenderLanes);
					else {
		...
		function commitRootImpl(root2, recoverableErrors, transitions, renderPriorityLevel) {
			...
			root2.finishedWork = null;
			root2.finishedLanes = NoLanes;
			if (finishedWork === root2.current) throw new Error("Cannot commit the same tree as before. This error is likely caused by a bug in React. Please file an issue.");
			root2.callbackNode = null;
			root2.callbackPriority = NoLane;
			...
			if (!rootDidHavePassiveEffects) commitDoubleInvokeEffectsInDEV(root2.current, false);
			onCommitRoot(finishedWork.stateNode, renderPriorityLevel);
			if (isDevToolsPresent) root2.memoizedUpdaters.clear();
			onCommitRoot$1();
			ensureRootIsScheduled(root2, now());
			...
			if (includesSomeLane(pendingPassiveEffectsLanes, SyncLane) && root2.tag !== LegacyRoot) flushPassiveEffects();
			remainingLanes = root2.pendingLanes;
		...
		function flushPassiveEffectsImpl() {
			...
			executionContext = prevExecutionContext;
			flushSyncCallbacks();
			if (didScheduleUpdateDuringPassiveEffects) if (root2 === rootWithPassiveNestedUpdates) nestedPassiveUpdateCount++;
			else {
				nestedPassiveUpdateCount = 0;
		...
		function captureCommitPhaseErrorOnRoot(rootFiber, sourceFiber, error2) {
			var root2 = enqueueUpdate(rootFiber, createRootErrorUpdate(rootFiber, createCapturedValueAtFiber(error2, sourceFiber), SyncLane), SyncLane);
			var eventTime = requestEventTime();
			if (root2 !== null) {
		...
		function invokeEffectsInDev(firstChild, fiberFlags, invokeEffectFn) {
			...
			while (current2 !== null) {
				...
				if (current2 !== subtreeRoot && current2.child !== null && primarySubtreeFlag !== NoFlags) current2 = current2.child;
				else {
					if ((current2.flags & fiberFlags) !== NoFlags) invokeEffectFn(current2);
					if (current2.sibling !== null) current2 = current2.sibling;
					else current2 = subtreeRoot = current2.return;
				}
			}
		}
		...
		function restorePendingUpdaters(root2, lanes) {
			if (isDevToolsPresent) root2.memoizedUpdaters.forEach(function(schedulingFiber) {
		...
		function warnIfSuspenseResolutionNotWrappedWithActDEV(root2) {
			if (root2.tag !== LegacyRoot && isConcurrentActEnvironment() && ReactCurrentActQueue$1.current === null) error("A suspended resource finished loading inside a test, but the event was not wrapped in act(...).\n\nWhen testing, code that resolves suspended data should be wrapped into act(...):\n\nact(() => {\n  /* finish loading suspended data */\n});\n/* assert on the output */\n\nThis ensures that you're testing the behavior the user would see in the browser. Learn more at https://reactjs.org/link/wrap-tests-with-act");
		}
		...
		function createHydrationContainer(initialChildren, callback, containerInfo, tag, hydrationCallbacks, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix, onRecoverableError, transitionCallbacks) {
			var root2 = createFiberRoot(containerInfo, tag, true, initialChildren, hydrationCallbacks, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix, onRecoverableError);
		...
		function attemptSynchronousHydration$1(fiber) {
			switch (fiber.tag) {
				case HostRoot:
					...
					if (isRootDehydrated(root2)) flushRoot(root2, getHighestPriorityPendingLanes(root2));
					break;
				case SuspenseComponent:
					flushSync(function() {
						var root3 = enqueueConcurrentRenderForLane(fiber, SyncLane);
						if (root3 !== null) scheduleUpdateOnFiber(root3, fiber, SyncLane, requestEventTime());
					});
		...
		function attemptContinuousHydration$1(fiber) {
			...
			if (root2 !== null) scheduleUpdateOnFiber(root2, fiber, lane, requestEventTime());
		...
		var copyWithRenameImpl = function(obj, oldPath, newPath, index2) {
			...
			} else updated[oldKey] = copyWithRenameImpl(obj[oldKey], oldPath, newPath, index2 + 1);
			return updated;
		};
		...
		ReactDOMHydrationRoot.prototype.render = ReactDOMRoot.prototype.render = function(children) {
			...
			if (root2 === null) throw new Error("Cannot update an unmounted root.");
		...
		function createRoot(container, options2) {
			...
			if (options2 !== null && options2 !== void 0) {
				if (options2.hydrate) warn("hydrate through createRoot is deprecated. Use ReactDOMClient.hydrateRoot(container, <App />) instead.");
				else if (typeof options2 === "object" && options2 !== null && options2.$$typeof === REACT_ELEMENT_TYPE) error("You passed a JSX element to createRoot. You probably meant to call root.render instead. Example usage:\n\n  let root = createRoot(domContainer);\n  root.render(<App />);");
				if (options2.unstable_strictMode === true) isStrictMode = true;
				if (options2.identifierPrefix !== void 0) identifierPrefix = options2.identifierPrefix;
				if (options2.onRecoverableError !== void 0) onRecoverableError = options2.onRecoverableError;
				if (options2.transitionCallbacks !== void 0) options2.transitionCallbacks;
			}
		...
		function legacyRenderSubtreeIntoContainer(parentComponent, children, container, forceHydrate, callback) {
			...
			var root2;
			if (!maybeRoot) root2 = legacyCreateRootFromDOMContainer(container, children, parentComponent, callback, forceHydrate);
			else {
				root2 = maybeRoot;
				if (typeof callback === "function") {
					var originalCallback = callback;
...
//#endregion
//#region react-dom.js
var react_dom_default = __commonJS({ "../../node_modules/react-dom/index.js"(exports, module) {
